...
from keras.layers import Input, Conv2D, MaxPooling2D, Dense, BatchNormalization, Activation, Lambda, Flatten, AveragePooling2D, UpSampling2D, Concatenate
...
def build(input_shape, num_units_start, num_units_top, feature_extractor):
    ...
    output1 = model(inpt1)
    output2 = model(inpt2)
    concat = Concatenate(axis=-1)([output1, output2])
    abs_diff = Lambda(abs_difference)([output1, output2])
    distance = Lambda(euclidean_distance)([output1, output2])

    mlp = Dense(1024, activation='elu', kernel_regularizer=l2(0.0001), bias_regularizer=l2(0.0001))(concat)
